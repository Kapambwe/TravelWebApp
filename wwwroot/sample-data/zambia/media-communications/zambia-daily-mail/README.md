# Zambia Daily MailWebApp Sample Data Documentation

This directory contains sample JSON data files for all Mock Services used by the Zambia Daily MailWebApp. These files provide realistic sample data for development, testing, and demonstration purposes.

## Overview

The Zambia Daily MailWebApp uses **51 Mock Services** that provide sample data when the application is running in development mode with `UseMockServices: true` in the configuration.

## Directory Structure

```
wwwroot/sample-data/zambia-daily-mail/
├── README.md (this file)
├── zambia-daily-mail-service.json
...
├── zambia-daily-mail-period-service.json
├── audit-log-service.json
├── current-user-context.json
├── zambia-daily-mail-settings-service.json
...
### Core Zambia Daily Mail Services

#### 1. MockZambia Daily MailService
**File:** `zambia-daily-mail-service.json`
**Purpose:** Main zambia-daily-mail data including accounts, balances, and account types
...
#### 8. MockZambia Daily MailPeriodService
**File:** `zambia-daily-mail-period-service.json`
...
#### 11. MockZambia Daily MailSettingsService
**File:** `zambia-daily-mail-settings-service.json`
**Purpose:** System-wide zambia-daily-mail settings
...
The Zambia Daily MailWebApp uses these mock services when configured with `UseMockServices: true` in `appsettings.json`:
...
All mock services are registered in `ServiceCollectionExtensions.cs` using the `AddAdvancedZambia Daily MailMockServices()` extension method.
...
1. Locate the appropriate JSON file in `wwwroot/sample-data/zambia-daily-mail/`
...
- **Method:** `AddAdvancedZambia Daily MailServices(useMockData: true)`
...
**Version:** 1.0
**Maintained By:** Zambia Daily Mail-As-A-Service Development Team
