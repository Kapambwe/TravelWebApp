# Times PrintpakWebApp Sample Data Documentation

This directory contains sample JSON data files for all Mock Services used by the Times PrintpakWebApp. These files provide realistic sample data for development, testing, and demonstration purposes.

## Overview

The Times PrintpakWebApp uses **51 Mock Services** that provide sample data when the application is running in development mode with `UseMockServices: true` in the configuration.

## Directory Structure

```
wwwroot/sample-data/times-printpak/
├── README.md (this file)
├── times-printpak-service.json
...
├── times-printpak-period-service.json
├── audit-log-service.json
├── current-user-context.json
├── times-printpak-settings-service.json
...
### Core Times Printpak Services

#### 1. MockTimes PrintpakService
**File:** `times-printpak-service.json`
**Purpose:** Main times-printpak data including accounts, balances, and account types
...
#### 8. MockTimes PrintpakPeriodService
**File:** `times-printpak-period-service.json`
...
#### 11. MockTimes PrintpakSettingsService
**File:** `times-printpak-settings-service.json`
**Purpose:** System-wide times-printpak settings
...
The Times PrintpakWebApp uses these mock services when configured with `UseMockServices: true` in `appsettings.json`:
...
All mock services are registered in `ServiceCollectionExtensions.cs` using the `AddAdvancedTimes PrintpakMockServices()` extension method.
...
1. Locate the appropriate JSON file in `wwwroot/sample-data/times-printpak/`
...
- **Method:** `AddAdvancedTimes PrintpakServices(useMockData: true)`
...
**Version:** 1.0
**Maintained By:** Times Printpak-As-A-Service Development Team
