# ZAMPALMWebApp Sample Data Documentation

This directory contains sample JSON data files for all Mock Services used by the ZAMPALMWebApp. These files provide realistic sample data for development, testing, and demonstration purposes.

## Overview

The ZAMPALMWebApp uses **51 Mock Services** that provide sample data when the application is running in development mode with `UseMockServices: true` in the configuration.

## Directory Structure

```
wwwroot/sample-data/zampalm/
├── README.md (this file)
├── zampalm-service.json
...
├── zampalm-period-service.json
├── audit-log-service.json
├── current-user-context.json
├── zampalm-settings-service.json
...
### Core ZAMPALM Services

#### 1. MockZAMPALMService
**File:** `zampalm-service.json`
**Purpose:** Main zampalm data including accounts, balances, and account types
...
#### 8. MockZAMPALMPeriodService
**File:** `zampalm-period-service.json`
...
#### 11. MockZAMPALMSettingsService
**File:** `zampalm-settings-service.json`
**Purpose:** System-wide zampalm settings
...
The ZAMPALMWebApp uses these mock services when configured with `UseMockServices: true` in `appsettings.json`:
...
All mock services are registered in `ServiceCollectionExtensions.cs` using the `AddAdvancedZAMPALMMockServices()` extension method.
...
1. Locate the appropriate JSON file in `wwwroot/sample-data/zampalm/`
...
- **Method:** `AddAdvancedZAMPALMServices(useMockData: true)`
...
**Version:** 1.0
**Maintained By:** ZAMPALM-As-A-Service Development Team
