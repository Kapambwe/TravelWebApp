# ZAMTELWebApp Sample Data Documentation

This directory contains sample JSON data files for all Mock Services used by the ZAMTELWebApp. These files provide realistic sample data for development, testing, and demonstration purposes.

## Overview

The ZAMTELWebApp uses **51 Mock Services** that provide sample data when the application is running in development mode with `UseMockServices: true` in the configuration.

## Directory Structure

```
wwwroot/sample-data/zamtel/
├── README.md (this file)
├── zamtel-service.json
...
├── zamtel-period-service.json
├── audit-log-service.json
├── current-user-context.json
├── zamtel-settings-service.json
...
### Core ZAMTEL Services

#### 1. MockZAMTELService
**File:** `zamtel-service.json`
**Purpose:** Main zamtel data including accounts, balances, and account types
...
#### 8. MockZAMTELPeriodService
**File:** `zamtel-period-service.json`
...
#### 11. MockZAMTELSettingsService
**File:** `zamtel-settings-service.json`
**Purpose:** System-wide zamtel settings
...
The ZAMTELWebApp uses these mock services when configured with `UseMockServices: true` in `appsettings.json`:
...
All mock services are registered in `ServiceCollectionExtensions.cs` using the `AddAdvancedZAMTELMockServices()` extension method.
...
1. Locate the appropriate JSON file in `wwwroot/sample-data/zamtel/`
...
- **Method:** `AddAdvancedZAMTELServices(useMockData: true)`
...
**Version:** 1.0
**Maintained By:** ZAMTEL-As-A-Service Development Team
