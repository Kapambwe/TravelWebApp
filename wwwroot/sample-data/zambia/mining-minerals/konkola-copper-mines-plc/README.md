# AccountingWebApp Sample Data Documentation

This directory contains sample JSON data files for all Mock Services used by the AccountingWebApp. These files provide realistic sample data for development, testing, and demonstration purposes.

## Overview

The AccountingWebApp uses **51 Mock Services** that provide sample data when the application is running in development mode with `UseMockServices: true` in the configuration.

## Directory Structure

```
wwwroot/sample-data/accounting/
├── README.md (this file)
...
**File:** `accounting-service.json`
**Purpose:** Main accounting data including accounts, balances, and account types
...
**File:** `accounting-period-service.json`
...
**File:** `accounting-settings-service.json`
**Purpose:** System-wide accounting settings
...
The AccountingWebApp uses these mock services when configured with `UseMockServices: true` in `appsettings.json`:
...
All mock services are registered in `ServiceCollectionExtensions.cs` using the `AddAdvancedAccountingMockServices()` extension method.
...
1. Locate the appropriate JSON file in `wwwroot/sample-data/accounting/`
...
- **Method:** `AddAdvancedAccountingServices(useMockData: true)`
