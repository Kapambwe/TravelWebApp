# Zambia AirwaysWebApp Sample Data Documentation

This directory contains sample JSON data files for all Mock Services used by the Zambia AirwaysWebApp. These files provide realistic sample data for development, testing, and demonstration purposes.

## Overview

The Zambia AirwaysWebApp uses **51 Mock Services** that provide sample data when the application is running in development mode with `UseMockServices: true` in the configuration.

## Directory Structure

```
wwwroot/sample-data/zambia-airways/
├── README.md (this file)
├── zambia-airways-service.json
...
├── zambia-airways-period-service.json
├── audit-log-service.json
├── current-user-context.json
├── zambia-airways-settings-service.json
...
### Core Zambia Airways Services

#### 1. MockZambia AirwaysService
**File:** `zambia-airways-service.json`
**Purpose:** Main zambia-airways data including accounts, balances, and account types
...
#### 8. MockZambia AirwaysPeriodService
**File:** `zambia-airways-period-service.json`
...
#### 11. MockZambia AirwaysSettingsService
**File:** `zambia-airways-settings-service.json`
**Purpose:** System-wide zambia-airways settings
...
The Zambia AirwaysWebApp uses these mock services when configured with `UseMockServices: true` in `appsettings.json`:
...
All mock services are registered in `ServiceCollectionExtensions.cs` using the `AddAdvancedZambia AirwaysMockServices()` extension method.
...
1. Locate the appropriate JSON file in `wwwroot/sample-data/zambia-airways/`
...
- **Method:** `AddAdvancedZambia AirwaysServices(useMockData: true)`
...
**Version:** 1.0
**Maintained By:** Zambia Airways-As-A-Service Development Team
