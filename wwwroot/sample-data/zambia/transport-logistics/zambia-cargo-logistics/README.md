# Zambia Cargo & LogisticsWebApp Sample Data Documentation

This directory contains sample JSON data files for all Mock Services used by the Zambia Cargo & LogisticsWebApp. These files provide realistic sample data for development, testing, and demonstration purposes.

## Overview

The Zambia Cargo & LogisticsWebApp uses **51 Mock Services** that provide sample data when the application is running in development mode with `UseMockServices: true` in the configuration.

## Directory Structure

```
wwwroot/sample-data/zambia-cargo-logistics/
├── README.md (this file)
├── zambia-cargo-logistics-service.json
...
├── zambia-cargo-logistics-period-service.json
├── audit-log-service.json
├── current-user-context.json
├── zambia-cargo-logistics-settings-service.json
...
### Core Zambia Cargo & Logistics Services

#### 1. MockZambia Cargo & LogisticsService
**File:** `zambia-cargo-logistics-service.json`
**Purpose:** Main zambia-cargo-logistics data including accounts, balances, and account types
...
#### 8. MockZambia Cargo & LogisticsPeriodService
**File:** `zambia-cargo-logistics-period-service.json`
...
#### 11. MockZambia Cargo & LogisticsSettingsService
**File:** `zambia-cargo-logistics-settings-service.json`
**Purpose:** System-wide zambia-cargo-logistics settings
...
The Zambia Cargo & LogisticsWebApp uses these mock services when configured with `UseMockServices: true` in `appsettings.json`:
...
All mock services are registered in `ServiceCollectionExtensions.cs` using the `AddAdvancedZambia Cargo & LogisticsMockServices()` extension method.
...
1. Locate the appropriate JSON file in `wwwroot/sample-data/zambia-cargo-logistics/`
...
- **Method:** `AddAdvancedZambia Cargo & LogisticsServices(useMockData: true)`
...
**Version:** 1.0
**Maintained By:** Zambia Cargo & Logistics-As-A-Service Development Team
