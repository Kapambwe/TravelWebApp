# Nitrogen Chemicals ZambiaWebApp Sample Data Documentation

This directory contains sample JSON data files for all Mock Services used by the Nitrogen Chemicals ZambiaWebApp. These files provide realistic sample data for development, testing, and demonstration purposes.

## Overview

The Nitrogen Chemicals ZambiaWebApp uses **51 Mock Services** that provide sample data when the application is running in development mode with `UseMockServices: true` in the configuration.

## Directory Structure

```
wwwroot/sample-data/nitrogen-chemicals-zambia/
├── README.md (this file)
├── nitrogen-chemicals-zambia-service.json
...
├── nitrogen-chemicals-zambia-period-service.json
├── audit-log-service.json
├── current-user-context.json
├── nitrogen-chemicals-zambia-settings-service.json
...
### Core Nitrogen Chemicals Zambia Services

#### 1. MockNitrogen Chemicals ZambiaService
**File:** `nitrogen-chemicals-zambia-service.json`
**Purpose:** Main nitrogen-chemicals-zambia data including accounts, balances, and account types
...
#### 8. MockNitrogen Chemicals ZambiaPeriodService
**File:** `nitrogen-chemicals-zambia-period-service.json`
...
#### 11. MockNitrogen Chemicals ZambiaSettingsService
**File:** `nitrogen-chemicals-zambia-settings-service.json`
**Purpose:** System-wide nitrogen-chemicals-zambia settings
...
The Nitrogen Chemicals ZambiaWebApp uses these mock services when configured with `UseMockServices: true` in `appsettings.json`:
...
All mock services are registered in `ServiceCollectionExtensions.cs` using the `AddAdvancedNitrogen Chemicals ZambiaMockServices()` extension method.
...
1. Locate the appropriate JSON file in `wwwroot/sample-data/nitrogen-chemicals-zambia/`
...
- **Method:** `AddAdvancedNitrogen Chemicals ZambiaServices(useMockData: true)`
...
**Version:** 1.0
**Maintained By:** Nitrogen Chemicals Zambia-As-A-Service Development Team
